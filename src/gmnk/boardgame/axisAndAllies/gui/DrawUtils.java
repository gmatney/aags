...
/**
 * Provides draw* methods to the GameBoardGUI, taking into account the camera
...
public class DrawUtils {
	...
	public static void drawImage(BufferedImage image, Point destination) {
		//TODO add center of map
		
		g2d.drawImage(image
				,destination.x - cam.getX() 
				,destination.y - cam.getY()
				,cam.getZoomWidth()
				,cam.getZoomHeight()
				, null);
	}
	...
	public static void fillRect(int x, int y, int width, int height) {
		g2d.fillRect(
				(int) (x * (1 + cam.getZoomFactor() / 10) - cam.getX()),
				(int) (y * (1 + cam.getZoomFactor() / 10) - cam.getY()), width,
				height);
	}
	...
	public static void drawString(String string, int x, int y) {
		...
		for (int i = 1; i <= lines.length; i++) {
			g2d.drawString(lines[i - 1],
					(int) (x * (1 + cam.getZoomFactor() / 10) - cam.getX()),
					(int) (y * (1 + cam.getZoomFactor() / 10) - cam.getY() + i * 15));
		}
	}
}
